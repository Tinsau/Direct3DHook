// WindowFinderDlg.cpp : 定义 DLL 应用程序的导出函数。
//
...
BOOL CALLBACK SearchWindowDialogProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam);
...
extern "C"
{
    WINDOWFINDERDLG_API HWND WINAPI StartSearchWindowDlg(HWND app_main_hwnd)
    {
        long ret = StartSearchWindowDialog(HWND(app_main_hwnd));
...
)
{
	...
	wsprintf(szText, TEXT("%d"), screenpoint.x);
	SetDlgItemText(hwndDialog, IDC_STATIC_X_POS, szText);

	wsprintf(szText, TEXT("%d"), screenpoint.y);
...
long DisplayInfoOnFoundWindow(HWND hwndDialog, HWND hwndFoundWindow)
{
	...
	GetClassName(hwndFoundWindow, szClassName, 255);

	// Display some information on the found window.
	wsprintf
	(
		szText, TEXT("Window Handle == 0x%08X.\r\nClass Name : %s.\r\nRECT.left == %d.\r\nRECT.top == %d.\r\nRECT.right == %d.\r\nRECT.bottom == %d.\r\n"),
		hwndFoundWindow,
		szClassName,
		rect.left,
		rect.top,
...
)
{
	...
	case WM_INITDIALOG:
	{
		SetFinderToolImage(hwndDlg, TRUE);
		bRet = TRUE;
		break;
	}
